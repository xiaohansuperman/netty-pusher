...
public class PushClient {
    ...
    public void send(){
    	try {
			String name = "客户端发来一条Request";
            RequestProtocol protocol = new RequestProtocol();
            protocol.setName(name);
            ProtocolData data = new ProtocolData(ProtocolType.Request,protocol);
            byte[] p = data.toBytes();
    		...
    		while(sendBuffer.hasRemaining()) {
    			int writeSize=channel.write(sendBuffer);
    			count += writeSize;
    		}
    		System.out.println("客户端发送一条Request");
		} catch (IOException e) {
			e.printStackTrace();
		} catch (Exception e) {
			e.printStackTrace();
